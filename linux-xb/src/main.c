...
typedef struct
{
	...
	int  no_wait, write_tap;
	int be_sending, need_stop;
	uint64_t send_times_cnt;
	struct timeval first_snd_tv, last_snd_tv;


	int   enable_max_speed;
	int  pkt_file_type;
	char *config_file;
	int   burst_num;
	char  *if_name;
...
static t_run_info gt_run_info = 
{
	.pkt_file_type = -1,
	.cpu_idx = -1,
	.snd_interval = 1000000,
	.frequency = 1,
...
static const char *opt_remark[][2] = {
...
static void report_working_paras()
{


    gt_run_info.intvl.tv_sec = gt_run_info.snd_interval/1000000;
    gt_run_info.intvl.tv_usec = gt_run_info.snd_interval%1000000;
    
    printf("\n[working paras]\n");
    
    if (gt_run_info.enable_max_speed)
        printf("%sSPEED: MAX_SPEED\n", LINE_HDR);
    else
        printf("%sSPEED: interval=%lus %luus <==> (pps=%"PRIi64")\n"
        , LINE_HDR
        , gt_run_info.intvl.tv_sec, gt_run_info.intvl.tv_usec, gt_run_info.frequency);


    if (gt_run_info.burst_num>0)
        printf("%sMODE: burst %d times\n", LINE_HDR, gt_run_info.burst_num);
    else
...
static int check_args()
{
    ...
    if (gt_run_info.config_file==NULL)
    {
        printf("you must provide a config file\n");
        goto FAIL_EXIT;
    }

    if (gt_run_info.if_name==NULL)
    {
        printf("you must specify interface for sending packets\n");
        goto FAIL_EXIT;
    }
    
    if (gt_run_info.enable_max_speed) goto SUCC_EXIT;
    if (gt_run_info.snd_interval<=0)
    {
...
static int parse_and_check_args(int argc, char *argv[])
{
   int opt;
    while ((opt = getopt_long(argc, argv, "hHmMi:I:f:F:", my_options, NULL)) != -1)
    {
       switch (opt)
       {
           ...
           case 'F':
           case CONFIG_FILE: 
               gt_run_info.config_file = optarg;
               gt_run_info.pkt_file_type = 0;
               break;
               
           case SINGLE_BIN_PKT: 
               gt_run_info.config_file = optarg;
               gt_run_info.pkt_file_type = 1;
               break;
#if 0
           case CAP_PKT_FILE: 
               gt_run_info.config_file = optarg;
               gt_run_info.pkt_file_type = 2;
               break;
#endif
           ...
           case 'I':
           case INTERFACE:
               gt_run_info.if_name = optarg;
               break;

           case BIND_CPU:
               gt_run_info.cpu_idx = strtol(optarg, NULL, 0);
               break;
               
           case INTERVAL:
               gt_run_info.snd_interval = strtol(optarg, NULL, 0);
               break;


           case BURST_NUM:
               gt_run_info.burst_num = strtol(optarg, NULL, 0);
               break;


           case PPS:
               gt_run_info.frequency  = strtol(optarg, NULL, 0);
               if (gt_run_info.frequency>0)
                   gt_run_info.snd_interval = 1000000/gt_run_info.frequency;
               else
                   gt_run_info.snd_interval = 0;
               break;

           case SET_SRC_MAC:
               mac_str2n(gt_run_info.src_mac, optarg);
               gt_run_info.has_src_mac = 1;
               break;

           case SET_DST_MAC:
               mac_str2n(gt_run_info.dst_mac, optarg);
               gt_run_info.has_dst_mac = 1;
               break;
           ...
           case ENABLE_MAX_SPEED:
               gt_run_info.enable_max_speed = 1;
               break;
...
static struct timeval time_a_between_b2(struct timeval a, struct timeval b)
{
    struct timeval ret;
...
static void report_snd_summary()
{
...
static void init_bits(void *field_addr, t_rule *pt_rule)
{
...
static void init_u8_n(void *data, t_rule *pt_rule)
{
    *(uint8_t *)data=pt_rule->base_value[0];
}

static void init_u16_n(void *data, t_rule *pt_rule)
{
    *(uint16_t *)data=*(uint16_t *)(pt_rule->base_value);
}

static void init_u32_n(void *data, t_rule *pt_rule)
{
    *(uint32_t *)data=*(uint32_t *)(pt_rule->base_value);
}

static void init_rule_field(t_stream *pt_stream, t_rule *pt_rule)
{
    if (pt_rule->bits_len)
    {
...
static void update_u8_n(void *data, t_rule *pt_rule)
{
...
static void update_u16_n(void *data, t_rule *pt_rule)
{
...
static void update_u32_n(void *data, t_rule *pt_rule)
{
...
static void check_sum_proc(t_stream *pt_stream)
{
...
static void update_bits(void *field_addr, t_rule *pt_rule)
{
...
static void update_rule_field(t_stream *pt_stream, t_rule *pt_rule)
{
    if (pt_rule->bits_len)
    {
...
static void rule_fileds_init(t_stream *pt_stream)
{
    int i;
    t_rule *pt_rule;
...
static void rule_fileds_update(t_stream *pt_stream)
{
    int i;
    t_rule *pt_rule;
    for (i=0; i<pt_stream->rule_num; i++)
...
static void rule_fileds_init_all_pkt()
{
    int i;
    t_stream *pt_stream;
...
static int send_pkt(void *arg)
{
    ...
    if (gt_run_info.cpu_idx>=0)
    {
        ret = set_thread_cpu_affinity(pthread_self(), gt_run_info.cpu_idx, gt_run_info.cpu_idx);
        if (ret)
            ERR_DBG_PRINT("bind thread to cpu %d failed", gt_run_info.cpu_idx);
    }

    //cpu_freq = get_cpu_freq();
    
    gettimeofday(&cur_tv, NULL);
    gt_run_info.first_snd_tv = cur_tv;
    next_snd_tv = cur_tv;
    goto SND_PKT;

/* Send down the packet */
while (!gt_run_info.need_stop)
{
   if (gt_run_info.enable_max_speed)
    {
        //gettimeofday(&cur_tv, NULL);
    ...
    {   
       gettimeofday(&cur_tv, NULL);
    ...
    } while (!gt_run_info.need_stop && time_a_smaller_than_b(&cur_tv, &next_snd_tv));


        if (gt_run_info.need_stop) goto exit;

        gt_run_info.last_snd_tv = cur_tv;
        
            SND_PKT:
            while (g_apt_streams[i]->selected==0 && !gt_run_info.send_all)
            {
                i = (i+1)%nr_cur_stream;
                if (gt_run_info.need_stop) goto exit;
            }
        ...
        gt_run_info.send_times_cnt++;
        i = (i+1)%nr_cur_stream;
        if (gt_run_info.burst_num>0 && gt_run_info.send_times_cnt>=gt_run_info.burst_num)
        {
            goto exit;
        }


        if (gt_run_info.enable_max_speed)  continue;


        next_snd_tv.tv_sec+=gt_run_info.intvl.tv_sec;
        next_snd_tv.tv_usec+=gt_run_info.intvl.tv_usec;
...
exit:
    if (gt_run_info.enable_max_speed)
          gettimeofday(&gt_run_info.last_snd_tv, NULL);
    report_snd_summary();
    gt_run_info.be_sending = 0;
return 0;
}


static void create_thread(pthread_t *thread, void *fn, void *arg)
{
    pthread_create(thread, NULL, fn, arg);
}


static void register_sig_act(int sig, void *sig_proc_func)
{
...
{
    gt_run_info.need_stop = 1;
}


static int load_packets()
{
    if (0==gt_run_info.pkt_file_type)
        return load_config_file(gt_run_info.config_file
        ,gt_run_info.has_src_mac?gt_run_info.src_mac:NULL
        ,gt_run_info.has_dst_mac?gt_run_info.dst_mac:NULL);

    if (1==gt_run_info.pkt_file_type)
        return load_bin_packet_file(gt_run_info.config_file
        ,gt_run_info.has_src_mac?gt_run_info.src_mac:NULL
        ,gt_run_info.has_dst_mac?gt_run_info.dst_mac:NULL);
...
static int start_up_tap_if(const char *tap)
{
    char cmd_str[128];
...
static int prepare_tap_if(char *tap_name)
{
    struct ifreq ifr;
    int fd, ret;
...
static int has_sendable_pkt(void)
{
    int i;
    t_stream *pt_stream;

    if (gt_run_info.send_all) goto HAS_SENDABLE_PKT;
	...
	if (0==gt_run_info.pkt_file_type)
        ...
        ,gt_run_info.config_file);
	else
		printf("\n\n\n"
		"**** Error: No sendable packet found. ****\n"
        "        are you sure file %s contains a valid raw binary packet?\n"
        ,gt_run_info.config_file);
    return 0;
